...
class LearnedSimulator(nn.Module):
  ...
  def _compute_graph_connectivity(self, node_features, nparticles_per_example,
                                   radius, add_self_edges=True):
    batch_ids = torch.cat(
        [torch.LongTensor([i for _ in range(n)])
         for i, n in enumerate(nparticles_per_example)]).to(self._device)
    edge_index = radius_graph(
        node_features, r=radius, batch=batch_ids,
        loop=add_self_edges, max_num_neighbors=128)
    receivers = edge_index[0, :]
    senders = edge_index[1, :]
    return receivers, senders

  def _encoder_preprocessor(self, position_sequence, nparticles_per_example,
                              particle_types, material_property=None):
  ...
  def predict_positions(self, current_positions, nparticles_per_example,
                         particle_types, material_property=None):
    if material_property is not None:
  ...
  def predict_accelerations(self, next_positions, position_sequence_noise,
                             position_sequence, nparticles_per_example,
                             particle_types, material_property=None):
...
def time_diff(position_sequence):
